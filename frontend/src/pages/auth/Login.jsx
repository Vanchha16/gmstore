...
export default function Login() {
  ...
  const handleGoogleCredentialResponse = async (response) => {
    ...
    setLoading(true)
    try {
      // response.credential contains the real cryptographically-signed Google JWT ID Token!
    ...
    <div className="flex min-h-screen items-center justify-center px-4 bg-slate-950 auth-glow-container animate-fade-in">
      ...
      {loginSuccess && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-slate-950/80 backdrop-blur-md animate-fade-in">
          ...
          <h2 className="text-xl font-bold text-slate-100 tracking-tight animate-fade-in">Login Successful</h2>
      ...
      <div className="w-full max-w-md rounded-3xl border border-slate-900 bg-slate-900/35 p-8 shadow-2xl backdrop-blur-md animate-card-entrance">
        <div className="mb-6 text-center">
          <h1 className="text-3xl font-extrabold text-slate-100 tracking-tight">Sign In</h1>
          <p className="mt-2 text-sm text-slate-500">Access your GM Store account and game library</p>
        </div>
        ...
        <form onSubmit={submit} className="flex flex-col gap-5">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full rounded-xl border border-slate-700 bg-slate-800/50 px-4 py-2.5 text-sm text-slate-100 outline-none placeholder:text-slate-600 focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition ${errors.password ? 'border-red-500' : ''}`}
            />
        ...
        <div className="mt-6 text-center text-xs text-slate-500">
          New to GM Store?{' '}
          <Link to="/register" className="text-violet-455 hover:underline font-semibold transition-colors">
            Create an Account
          </Link>
        </div>
      </div>
